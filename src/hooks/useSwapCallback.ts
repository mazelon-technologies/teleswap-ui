...
export function swapErrorToUserReadableMessage(error: any): string {
  ...
  switch (reason) {
    ...
    case 'TelefyRouter: INSUFFICIENT_OUTPUT_AMOUNT':
    case 'TelefyRouter: EXCESSIVE_INPUT_AMOUNT':
    ...
    case 'STF':
      return t`This transaction will not succeed due to price movement. Try increasing your slippage tolerance. Note: fee on transfer and rebase tokens are incompatible with Telefy.`
    case 'TF':
      return t`The output token cannot be transferred. There may be an issue with the output token. Note: fee on transfer and rebase tokens are incompatible with Telefy.`
    default:
